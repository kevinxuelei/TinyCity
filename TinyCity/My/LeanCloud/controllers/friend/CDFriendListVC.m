...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *bar = [[UIBarButtonItem alloc] initWithTitle:@"我" style:(UIBarButtonItemStylePlain) target:self action:@selector(clickToMe)];
    UIBarButtonItem *nextBar = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:(UIBarButtonItemStylePlain) target:self action:@selector(click)];
    self.navigationItem.leftBarButtonItems = @[nextBar,bar];
    [self setupTableView];
    [self refresh];
}
#pragma mark -- 返回
...
- (void)clickToMe
{
    CDProfileVC *pro = [[CDProfileVC alloc] init];
